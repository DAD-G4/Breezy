...
import { getProfileByUsername, getFollowers, follow, unfollow } from "@/services/users";
...
export default function FollowersPage({ params }) {
  ...
  const fetchFollowers = async () => {
    ...
    try {
      ...
      const data = await getFollowers(u.id);
      setUsers(data.users || []);
    } catch (err) {
    ...
    <AppShell>
      <div className="p-4 flex flex-col gap-4">
        ...
        {!loading && !error && users.length === 0 && (
          <p className="text-center text-gray-500 dark:text-gray-400 py-8">{t("followersPage.empty")}</p>
        )}
        {!loading && !error && users.length > 0 && (
