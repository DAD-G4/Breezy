...
function NavItem({ href, label, icon, active, onClick, hasNotif }) {
  const base =
    "flex items-center gap-4 px-3 py-3 rounded-full transition-colors text-deep-space-blue dark:text-papaya-whip hover:bg-black/5 dark:hover:bg-white/5 justify-center lg:justify-start relative";
  ...
  const content = (
    <>
      <div className="relative">
        {icon}
        {/* point rouge sur l'icône si on a une notification */}
        {hasNotif && (
